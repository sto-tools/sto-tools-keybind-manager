 ...
 * @property {number | null} button
 * @property {number} startX
 * @property {number} startY
 * @property {number} dragThreshold
 * @property {ReturnType<typeof setTimeout> | null} pendingClickTimer
 * @property {number | null} pendingClickButton
 * @property {string | null} pendingClickGesture
 */
...
export default class KeyCaptureService extends ComponentBase {
  ...
  handleKeyUp(event) {
    ...
    if (!this.hasCapturedValidKey) {
      ...
      if (
        this.isModifier(event.code) &&
        this.pressedCodes.has(event.code) &&
        this.pressedCodes.size === 1
      ) {
  ...
  handleMouseDown(event) {
    ...
    if (
      this.mouseState.pendingClickTimer &&
  ...
  handleMouseUp(event) {
    ...
    if (this.mouseState.isDown && this.mouseState.button === event.button) {
      ...
      const hasMoved =
        deltaX > this.mouseState.dragThreshold ||
        deltaY > this.mouseState.dragThreshold;
  ...
  /** @param {number} button @param {'click' | 'doubleclick' | 'drag' | string} type */
  ...
  captureMouseGesture(gesture) {
    ...
    const modifiers = [...this.pressedCodes].filter((code) =>
      this.isModifier(code),
    );
    ...
    return [...codes]
      ...
      .map((code) => {
        ...
        if (code.startsWith("Control")) {
          if (locationSpecific)
            return code.endsWith("Left") ? "LCTRL" : "RCTRL";
          return "Control";
        }
        ...
        if (code.startsWith("Shift")) {
          if (locationSpecific)
            return code.endsWith("Left") ? "LSHIFT" : "RSHIFT";
          return "Shift";
        }
