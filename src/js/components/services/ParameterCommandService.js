...
export default class ParameterCommandService extends ComponentBase {
  ...
  /** @param {string} baseCommand @param {number} startTray @param {number} startSlot @param {number} endTray @param {number} endSlot @param {number} [active] */
  ...
  /** @param {string} baseCommand @param {number} tray @param {number} [active] */
  ...
  /** @param {number} active @param {number} startTray @param {number} startSlot @param {number} endTray @param {number} endSlot @param {number} backupStartTray @param {number} backupStartSlot @param {number} backupEndTray @param {number} backupEndSlot */
    ...
    if (startTray === endTray) {
      for (let slot = startSlot; slot <= endSlot; slot++) {
        ...
        if (active === 1) {
          ...
          commands.push(
            `+TrayExecByTrayWithBackup ${startTray} ${slot} ${backupStartTray} ${backupSlot}`,
          );
        } else {
          // Use explicit form when active=0
          commands.push(
            `TrayExecByTrayWithBackup ${active} ${startTray} ${slot} ${backupStartTray} ${backupSlot}`,
          );
        }
      }
    } else {
      ...
      for (let tray = startTray; tray <= endTray; tray++) {
        ...
        for (let slot = slotStart; slot <= slotEnd; slot++) {
          ...
          const backupSlotIndex =
            backupStartTray * 10 + backupStartSlot + commandIndex;
          ...
          if (
            backupTray <= backupEndTray &&
            (backupTray < backupEndTray || backupSlot <= backupEndSlot)
          ) {
            if (active === 1) {
              commands.push(
                `+TrayExecByTrayWithBackup ${tray} ${slot} ${backupTray} ${backupSlot}`,
              );
            } else {
              commands.push(
                `TrayExecByTrayWithBackup ${active} ${tray} ${slot} ${backupTray} ${backupSlot}`,
              );
            }
          } else {
            ...
            if (active === 1) {
              commands.push(
                `+TrayExecByTrayWithBackup ${tray} ${slot} ${backupStartTray} ${backupStartSlot}`,
              );
            } else {
              commands.push(
                `TrayExecByTrayWithBackup ${active} ${tray} ${slot} ${backupStartTray} ${backupStartSlot}`,
              );
            }
          }
          commandIndex++;
  ...
  /** @param {number} active @param {number} tray @param {number} backupTray */
  generateWholeTrayWithBackupCommands(active, tray, backupTray) {
    ...
    for (let slot = 0; slot <= 9; slot++) {
      if (active === 1) {
        // Use + form when active=1
        commands.push(
          `+TrayExecByTrayWithBackup ${tray} ${slot} ${backupTray} ${slot}`,
        );
      } else {
        // Use explicit form when active=0
        commands.push(
  ...
  /** @param {string} categoryId @param {string} commandId @param {ParameterCommandDefinition} commandDef @param {ParameterValues} [params] */
  async buildParameterizedCommand(
    categoryId,
    commandId,
    commandDef,
    params = {},
    ...
    /** @type {Record<string, ParameterBuilder>} */
    const builders = {
      targeting: (p) => {
        ...
        if (
          (commandId === "target" || commandId === "target_entity") &&
          p.entityName
        ) {
          return {
        ...
        const commandString =
          typeof commandDef.command === "string"
            ? commandDef.command
            : "Target";
      ...
      tray: async (p) => {
        ...
        if (commandId === "tray_range") {
          ...
          const commandStrings = this.generateTrayRangeCommands(
            baseCommand,
            startTray,
            startSlot,
            endTray,
        ...
        if (commandId === "whole_tray") {
          ...
          // Generate individual command strings, then parse through STOCommandParser for consistent format
          const commandStrings = this.generateWholeTrayCommands(
            baseCommand,
            tray,
            active,
          );
        ...
        if (commandId === "whole_tray_with_backup") {
          ...
          // Generate individual command strings, then parse through STOCommandParser for consistent format
          const commandStrings = this.generateWholeTrayWithBackupCommands(
            active,
            tray,
            backupTray,
          );
      ...
      communication: (p) => {
         ...
         */
        const fallbackVerb =
          commandDef && commandDef.command ? commandDef.command : "say";
      ...
      system: (p) => {
        ...
        if (
          ...
          commandId === "net_timing_graph_alpha" &&
          p.alpha !== undefined
        ) {
