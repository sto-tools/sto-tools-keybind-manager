...
export default class CommandUI extends UIComponentBase {
   ...
   *   parameterCommandUI?: import('./ParameterCommandUI.js').default | null,
  ...
  constructor({
    ...
    /** @type {Map<string, ValidationSeverity>} */
    this._toastSeverityByKey = new Map();

    this.document = resolveDocument(document);

    // Store current modal data for regeneration
    /** @type {{ errors: ValidationIssue[], warnings: ValidationIssue[], modalElement: HTMLElement } | null} */
  ...
  setupEventListeners() {
    ...
    this.addEventListener(
      ...
      async ({ key, severity, warnings = [], errors = [] }) => {
        ...
        /** @type {Record<ValidationSeverity, { icon: string, textKey: string, textDefault: string, color: string }>} */
        const mapping = {
          success: {
        ...
        const normalizedSeverity =
          severity === "warning" || severity === "error" ? severity : "success";
        const cfg = mapping[normalizedSeverity];
  ...
  async performImport() {
    ...
    const select = /** @type {HTMLSelectElement | null} */ (
      doc.getElementById("importSourceSelect")
    );
    const clearCheckbox = /** @type {HTMLInputElement | null} */ (
      doc.getElementById("clearDestinationBeforeImport")
    );
    ...
    try {
      ...
      if (detail === "Source has no commands to import") {
        message = this.i18n.t("source_has_no_commands");
      } else if (detail === "No compatible commands found for import") {
