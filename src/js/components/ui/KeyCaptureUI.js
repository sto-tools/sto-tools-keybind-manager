 ...
 *   preferences: { bindsetsEnabled?: boolean, bindToAliasMode?: boolean },
 *   activeBindset: string,
 *   bindsetNames: string[]
 * }} KeyCaptureCache
 */

/** @type {Record<string, { row: number, col: number, width: number }>} */
const keyboardPositions = KEY_POSITIONS;
/** @type {Record<string, { name: string, description: string }>} */
const mouseGestures = MOUSE_GESTURES;
...
export default class KeyCaptureUI extends UIComponentBase {
  ...
  onInit() {
    ...
    this.addEventListener("capture-start", () => this.handleCaptureStart());
    this.addEventListener(
      "update",
      /** @param {CaptureUpdate} d */ (d) => this.updateCapturedKeysDisplay(d),
    );
    this.addEventListener(
      "chord-captured",
      /** @param {CapturedChord} d */ (d) => this.handleChordCaptured(d),
    );
    ...
    if (this.eventBus) {
      this.eventBus.on(
        "modal:shown",
        /** @param {{ modalId?: string }} payload */ ({ modalId }) => {
  ...
  initializeModal() {
    ...
    const selector = /** @type {HTMLSelectElement | null} */ (
      this.document.getElementById("keyboardLayoutSelector")
    );
    ...
    const distinguishCheckbox = /** @type {HTMLInputElement | null} */ (
      this.document.getElementById("distinguishModifierSide")
    );
    const initialDistinguish = !!distinguishCheckbox?.checked;
    this.emit("keycapture:set-location-specific", {
      value: initialDistinguish,
    });
  ...
  handleKeyDuplication(sourceKey) {
    ...
    this.sourceKeyForDuplication = sourceKey ?? null;
  ...
  toggleVirtualModifier(keyCode) {
    ...
    const distinguishSides = distinguishCheckbox?.checked || false;
    ...
    const leftElement = this.document.querySelector(
      `[data-key-code="${leftKey}"]`,
    );
    const rightElement = this.document.querySelector(
      `[data-key-code="${rightKey}"]`,
    );
    ...
    const hasNonModifierKey =
      ...
      this.cache.selectedKey.split("+").some((p) => p && !modNames.includes(p));
    ...
    if (modifierType) {
      this.lastModifierSide[modifierType] = keyCode.endsWith("Right")
        ? "R"
        : "L";
    }
  }
  ...
  getActiveVirtualModifiers() {
    ...
    modifierTypes.forEach((type) => {
      const active = this.getActiveVirtualModifiersOfType(
        type,
        distinguishSides,
      );
      if (distinguishSides) {
  ...
  updatePreviewWithCurrentModifiers() {
    ...
    if (activeModifiers.length > 0) {
      ...
      if (this.cache.selectedKey) {
        ...
        const modPattern =
          /^(Ctrl|Alt|Shift|LCTRL|RCTRL|LALT|RALT|LSHIFT|RSHIFT)(\+|$)/i;
        const isOnlyModifiers = this.cache.selectedKey
          .split("+")
          .every((p) => modPattern.test(p));
        if (!isOnlyModifiers) {
  ...
  async confirmSelection() {
    ...
    try {
      ...
      if (this.isDuplicationMode) {
        ...
        if (result?.success) {
          const message = this.i18n.t("key_added", {
  ...
  shouldShowBindsetPicker() {
    ...
    return (
      prefs.bindsetsEnabled &&
      prefs.bindToAliasMode &&
  ...
  syncBindsetSelector() {
    const selector = /** @type {HTMLSelectElement | null} */ (
      this.document.getElementById("bindsetTargetSelector")
    );
    if (selector) {
  ...
  generateBindsetPickerHTML() {
    ...
    const bindsets =
      this.cache?.bindsetNames && this.cache.bindsetNames.length
        ? this.cache.bindsetNames
        : ["Primary Bindset"];

    const options = bindsets
      .map((name) => {
  ...
  updateKeyboardLayout() {
    ...
    this.currentKeyboard = JSON.parse(
      JSON.stringify(getKeyboardLayout(this.selectedLayout)),
    );
  ...
  renderVirtualKeyboard() {
    ...
    /**
     * @param {KeyboardRows} dict
     * @param {number} row
     * @param {PositionedKey} keyData
     */
    ...
    Object.entries(keyboardPositions).forEach(([keyCode, pos]) => {
      const target =
        pos.col >= 22
          ? mouseRows
          : pos.col >= 17
    ...
    const maxRow = Math.max(
      ...Object.values(keyboardPositions).map((p) => p.row),
    );

    /**
     * @param {KeyboardRows} rowsDict
  ...
  updateModifierHighlighting() {
    ...
    if (currentlyActive.length > 0) {
      if (distinguishSides) {
        /** @type {Set<ModifierType>} */
        const handledTypes = new Set();
        /** @type {Record<ModifierSide, 'Left' | 'Right'>} */
        ...
        currentlyActive.forEach((keyCode) => {
          ...
          const preferredKey =
            type === "ctrl"
              ? `Control${sidePref[preferredSide]}`
              : type === "alt"
                ? `Alt${sidePref[preferredSide]}`
          ...
          if (!currentlyActive.includes(chosenKey)) {
            const altKey =
              preferredSide === "L"
                ? preferredKey.replace("Left", "Right")
                : preferredKey.replace("Right", "Left");
          ...
          if (chosenKey === leftKey) {
            const rightEl = this.document.querySelector(
              `[data-key-code="${rightKey}"]`,
            );
            rightEl?.classList.remove("selected");
          } else {
            const leftEl = this.document.querySelector(
              `[data-key-code="${leftKey}"]`,
            );
  ...
  highlightSelectedKeyOnKeyboard(chord) {
    ...
    keys.forEach((key) => {
      ...
      if (key === "Ctrl" && !distinguishSides) {
        ...
        for (const [code, keyInfo] of Object.entries(
  ...
  enableConfirmButton() {
    const btn = /** @type {HTMLButtonElement | null} */ (
      this.document.getElementById("confirm-key-selection")
    );
    if (btn) {
      btn.disabled = false;
    }
  ...
  updateChordWithLocationSpecific(useLocationSpecific) {
    ...
    if (useLocationSpecific) {
      ...
      updatedChord = updatedChord
        .replace(
          /\bCtrl\b/g,
          this.lastModifierSide.ctrl === "R" ? "RCTRL" : "LCTRL",
        )
        .replace(
          /\bAlt\b/g,
          this.lastModifierSide.alt === "R" ? "RALT" : "LALT",
        )
        .replace(
          /\bShift\b/g,
          this.lastModifierSide.shift === "R" ? "RSHIFT" : "LSHIFT",
        );
    } else {
