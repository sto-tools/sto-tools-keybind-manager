...
class DevMonitor {
  constructor() {
     ...
     *   selectorUsageCount: Map<string, number>,
  ...
  enableI18nTracking() {
    if (!this.isDevelopment) {
      console.warn(
        "DevMonitor: I18n tracking only available in development mode",
      );
      return false;
    }
  ...
  trackI18nUsage(key) {
    ...
    this.i18nStats.keyUsageCount.set(
      normalizedKey,
      (this.i18nStats.keyUsageCount.get(normalizedKey) || 0) + 1,
    );
  ...
  enableCSSTracking() {
    if (!this.isDevelopment) {
      console.warn(
        "DevMonitor: CSS tracking only available in development mode",
      );
      return false;
    }
    ...
    sheets.forEach((sheet, sheetIndex) => {
      try {
        ...
        if (
          ...
          !sheet.href.startsWith("file://")
        ) {
          console.log(
            `[DevMonitor] Skipping external stylesheet: ${sheet.href}`,
          );
          return;
        }
        ...
        console.log(
          `[DevMonitor] Found ${rules.length} rules in sheet ${sheetIndex}`,
        );
          ...
          if (rule.type === CSSRule.STYLE_RULE) {
            ...
            try {
              ...
              if (elements.length > 0) {
                ...
                this.cssStats.selectorUsageCount.set(
                  selector,
                  (this.cssStats.selectorUsageCount.get(selector) || 0) + 1,
                );
              ...
              console.warn(
                `[DevMonitor] Invalid selector "${selector}":`,
                message,
              );
            }
          }
        ...
        console.warn(
          `[DevMonitor] Can't access stylesheet ${sheetIndex}:`,
          message,
        );
      }
    });
  ...
  getI18nStats() {
    ...
    const stats = {
      summary: {
        ...
        totalUsages: Array.from(this.i18nStats.keyUsageCount.values()).reduce(
          (a, b) => a + b,
          0,
        ),
      },
      ...
      keyUsageCount: Object.fromEntries(
        Array.from(this.i18nStats.keyUsageCount.entries()).sort(
          (a, b) => b[1] - a[1],
        ),
      ),
  ...
  getCSSStats() {
    ...
    const stats = {
      summary: {
        ...
        lastChecked: this.cssStats.lastChecked
          ? new Date(this.cssStats.lastChecked).toISOString()
          : null,
      },
      ...
      selectorUsageCount: Object.fromEntries(
        Array.from(this.cssStats.selectorUsageCount.entries()).sort(
          (a, b) => b[1] - a[1],
        ),
      ),
...
`;
  ...
  checkCSSNow() {
    if (!this.isDevelopment) {
      console.warn(
        "DevMonitor: CSS checking only available in development mode",
      );
      return false;
    }
