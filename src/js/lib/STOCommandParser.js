 ...
 *   generateDisplayText?: (params: Record<string, any>) => any
 * }} CommandPattern
 * @typedef {{
...
export class STOCommandParser {
  /**
   * @param {typeof import('../core/eventBus.js').default | null} [eventBus]
   * @param {{
   *   enableCache?: boolean,
   *   maxCacheSize?: number,
    ...
    return {
        ...
        patterns: [
          {
            ...
            regex:
              /^(\+TrayExecByTrayWithBackup)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)$/i,
            weight: 90,
            signature:
          ...
          },
          {
            ...
            regex:
              /^(TrayExecByTrayWithBackup)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)$/i,
            weight: 89,
            signature:
              "TrayExecByTrayWithBackup(active: number, tray: number, slot: number, backup_tray: number, backup_slot: number)",
            extractParams: (match) => ({
      ...
      MovementCommands: {
        patterns: [
          {
            regex:
              /^\+(fullimpulse|reverse|turn_left|turn_right|up|down|left|right|forward|backward)$/i,
            weight: 25,
  ...
  /** @param {string} commandString @param {Record<string, any>} [options] */
  ...
  /** @param {string} command @param {number} index @param {Record<string, any>} [options] */
  parseIndividualCommand(command, index, options = {}) {
    // Sort signatures by weight for performance (TrayExec checked first)
    const sortedSignatures = Object.entries(this.signatures).sort((a, b) => {
    ...
    for (const [signatureName, signature] of sortedSignatures) {
      for (const pattern of signature.patterns) {
        ...
        if (match) {
          ...
          const displayText =
            options.generateDisplayText !== false && pattern.generateDisplayText
  ...
  trackCommandFrequency(commandString) {
    ...
    if (
      count >= this.options.hotPathThreshold &&
      !this.hotPathCache.has(commandString)
    ) {
  ...
  /** @param {string} commandString @param {any} result */
  ...
  /** @param {string} operation @param {number} startTime */
