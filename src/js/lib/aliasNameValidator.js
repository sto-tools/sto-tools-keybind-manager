    ...
    if (Array.isArray(def.patterns)) {
      def.patterns.forEach((p) => {
        ...
        if (altMatch) {
          altMatch[1]
            .split("|")
            .forEach((cmd) => tokens.add(cmd.toLowerCase()));
        }
      });
    }
  });
...
const RESERVED_COMMANDS = (() => {
  ...
  Object.values(SIGNATURE_DEFINITIONS).forEach((def) => {
    const opts = /** @type {string[]} */ (
      def.parameters?.command_type?.options ||
        def.parameters?.verb?.options ||
        []
    );
 ...
 */
export function generateBindToAliasName(
  environment,
  keyName,
  bindsetName = null,
) {
  // Sanitize environment name (should already be clean, but just in case)
  const cleanEnv = (environment || "space")
    .toLowerCase()
    .replace(/[^a-z0-9]/g, "");
