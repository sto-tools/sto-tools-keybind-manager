 ...
 * @param {I18nLike} i18n - Internationalization object with t() method
 * @param {{ eventBus?: typeof eventBus }} [options] - Additional options
 * @returns {Promise<CommandRecord>} Rich command object for display
 */
export async function enrichForDisplay(commandString, i18n, options = {}) {
  if (typeof commandString !== "string") {
    console.warn(
      "enrichForDisplay: commandString must be a string, got:",
      typeof commandString,
    );
  ...
  try {
    ...
    const parseResult = /** @type {any} */ (
      await request(bus, "parser:parse-command-string", {
        commandString,
    ...
    if (commandDef) {
      ...
      if (commandDef.customizable && richObject.parameters) {
        richObject.text = formatParameterizedCommand(
          commandDef,
          richObject.parameters,
        );
    ...
    if (i18n && typeof i18n.t === "function") {
      ...
      if (translationKey) {
        ...
        if (translated && translated !== translationKey) {
          ...
          if (
            typeof parsedCommand.displayText === "object" &&
            parsedCommand.displayText?.params
          ) {
            const { tray, slot, backup_tray, backup_slot } =
              parsedCommand.displayText.params;
            let paramSuffix = "";
    ...
    console.warn(
      "enrichForDisplay: Failed to parse command:",
      commandString,
      error,
    );
    return createFallbackRichObject(commandString, i18n);
 ...
 * @param {string|CommandRecord|null|undefined} cmdOrObj - Command string or rich object
 ...
 * @param {string|CommandRecord|null|undefined} cmdOrObj - Command string or rich object
 * @param {{ eventBus?: typeof eventBus }} [options] - Options including eventBus
...
export async function normalizeToOptimizedString(cmdOrObj, options = {}) {
  ...
  try {
    ...
    console.warn(
      "normalizeToOptimizedString: Failed to parse command for optimization:",
      commandString,
      error,
    );
  }
 ...
 * @param {Array<string|CommandRecord>|string|CommandRecord|null|undefined} commands - Mixed array of strings and objects, or single item
...
export function normalizeToStringArray(commands) {
  ...
  return commands.map(normalizeToString).filter((cmd) => cmd.length > 0);
}

/**
 * Create a fallback rich object when parsing fails
 * @param {*} commandString - The command that failed to parse
 * @param {I18nLike} i18n - Internationalization object
 * @returns {CommandRecord} Fallback rich object
 */
 ...
 * @param {CommandRecord} commandDef - Command definition
 * @param {CommandRecord} parameters - Extracted parameters
 * @returns {string} Formatted display text
 */
function formatParameterizedCommand(commandDef, parameters) {
 ...
 * @param {CommandRecord|null|undefined} parsedCommand - Parsed command from STOCommandParser
 * @param {CommandRecord|null|undefined} commandDef - Command definition if available
