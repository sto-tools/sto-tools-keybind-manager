...
export class KBFDecoder {
  ...
  constructor(options = {}) {
    /** @type {{ validateUtf8: boolean, strictMode: boolean, maxFileSize: number, [option: string]: any }} */
    this.options = {
      validateUtf8: true,
      strictMode: false,
  ...
  parseFile(content, options = {}) {
    ...
    try {
      ...
      this.addError(`Critical parsing error: ${errorObject.message}`, {
  ...
  validateFormat(content) {
    ...
    try {
      ...
      const validRecords = (layer2Content.match(validRecordPattern) || [])
        .length;
