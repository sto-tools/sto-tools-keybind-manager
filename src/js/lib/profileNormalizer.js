...
import {
  normalizeToString,
  normalizeToStringArray as _normalizeToStringArray,
...
/** @type {Record<string, { targetVersion: string, migrations: string[] }>} */
...
/** @param {string | null | undefined} currentVersion @param {string} targetVersion */
...
function optimizeTrayCommandSync(cmd) {
  ...
  const backupRegex =
    /^TrayExecByTrayWithBackup\s+1\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)$/i;
 ...
 * @param {any} profile - Profile to normalize
 * @returns {any} Normalized profile (mutates original)
 */
export function normalizeProfile(profile) {
  ...
  for (const step of migrationPath) {
    console.log(
      `[ProfileNormalizer] Migrating from ${step.fromVersion} to ${step.toVersion}`,
    );
  ...
  if (profile.aliases && typeof profile.aliases === "object") {
    for (const [aliasName, aliasData] of Object.entries(profile.aliases)) {
      if (aliasData && typeof aliasData === "object") {
        ...
        if (
          aliasData.type === "vfx-alias" ||
          aliasName.startsWith("dynFxSetFXExclusionList")
        ) {
          console.log(
            `[ProfileNormalizer] Removing old VFX alias: ${aliasName}`,
          );
  ...
  if (profile.aliases && typeof profile.aliases === "object") {
    for (const aliasName of vfxAliasNames) {
      if (
        ...
        typeof profile.aliases[aliasName] === "object"
      ) {
        ...
        if (aliasData.commands) {
          ...
          if (
            JSON.stringify(corrected) !== JSON.stringify(aliasData.commands)
          ) {
            console.log(
              `[ProfileNormalizer] Fixing VFX command spelling in alias: ${aliasName}`,
            );
...
function correctVFXCommandSpelling(commands) {
  ...
  if (typeof commands === "string") {
    return commands.replace(
      /\bdynFxSetFXExclusionList\b/g,
      "dynFxSetFXExlusionList",
    );
  }
...
function normalizeKeyCommands(keyCommands) {
  ...
  console.warn(
    "normalizeKeyCommands: Unknown format for key commands:",
    keyCommands,
  );
  return [];
}
...
function normalizeAliasCommands(aliasCommands) {
  ...
  // Unknown format, return empty array
  console.warn(
    "normalizeAliasCommands: Unknown format for alias commands:",
    aliasCommands,
  );
  return [];
}
 ...
 * @param {any} profile - Profile to check
