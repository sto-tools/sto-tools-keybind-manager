...
export function createRequestResponseFixture(eventBus, options = {}) {
  const { trackRequests = true, defaultTimeout = 5000 } = options;
  ...
  const fixture = {
    ...
    expectRequest: (topic, payload) => {
      ...
      if (reqs.length === 0) {
        throw new Error(
          `Expected request for topic '${topic}' but it was not found`,
        );
      }

      if (payload !== undefined) {
        const matchingRequest = reqs.find(
          (req) => JSON.stringify(req.payload) === JSON.stringify(payload),
        );
        if (!matchingRequest) {
          throw new Error(
            `Expected request for topic '${topic}' with payload ${JSON.stringify(payload)} but it was not found`,
          );
        }
      }
    },
    ...
    expectResponse: (topic, result) => {
      ...
      if (responses.length === 0) {
        throw new Error(
          `Expected response for topic '${topic}' but it was not found`,
        );
      }

      if (result !== undefined) {
        const matchingResponse = responses.find(
          (res) => JSON.stringify(res.result) === JSON.stringify(result),
        );
        if (!matchingResponse) {
          throw new Error(
            `Expected response for topic '${topic}' with result ${JSON.stringify(result)} but it was not found`,
          );
        }
      }
    },
    ...
    expectHandlerRegistered: (topic) => {
      if (!handlers.has(topic)) {
        throw new Error(
          `Expected handler for topic '${topic}' but it was not registered`,
        );
      }
    },
