...
describe("SelectionService Cached Validation with Bindsets Integration", () => {
  ...
  describe("Alias Validation with Bindsets", () => {
    ...
    it("should handle malformed alias data", async () => {
      ...
      expect(selectionService.validateAliasExists("MalformedAlias1")).toBe(
        true,
      ); // Still considered valid alias
      expect(selectionService.validateAliasExists("MalformedAlias2")).toBe(
        true,
      ); // Still considered valid alias
    });
  });
  ...
  describe("Bindset selection synchronization", () => {
    ...
    it("requests bindset activation before emitting key-selected when context is provided", async () => {
      ...
      await selectionService.selectKey("F1", "space", {
        bindset: "Combat Bindset",
      });

      const history = env.eventBusFixture.getEventHistory();
      const rpcIndex = history.findIndex(
        (entry) => entry.event === "rpc:bindset-selector:set-active-bindset",
      );
      const keyIndex = history.findIndex(
        (entry) => entry.event === "key-selected",
      );
    ...
    it("resets to Primary bindset when no bindset context is provided", async () => {
      ...
      const rpcEvents = env.eventBusFixture.getEventsOfType(
        "rpc:bindset-selector:set-active-bindset",
      );
      expect(rpcEvents.length).toBeGreaterThan(0);
      expect(rpcEvents[rpcEvents.length - 1].data.payload.bindset).toBe(
        "Primary Bindset",
      );
