...
describe("Bind-to-Alias Mode Integration", () => {
  ...
  beforeEach(async () => {
    ...
    const mockI18n = {
      t: vi.fn((key) => {
        const translations = {
          export_generated_aliases_note:
            "Keybind lines that call generated aliases",
          export_alias_definitions_note:
            "(Alias definitions are in the alias file)",
          export_separator:
            "==============================================================================",
  ...
  describe("Alias Name Generation", () => {
    it("should generate valid alias names for keybinds", async () => {
      const { generateBindToAliasName } = await import(
        "../../src/js/lib/aliasNameValidator.js"
      );
      ...
      const testCases = [
        ...
        { env: "space", key: "Ctrl+A", expected: "sto_kb_space_ctrl_a" },
        {
          env: "space",
          key: "Shift+Space",
          expected: "sto_kb_space_shift_space",
        },
        ...
        { env: "space", key: "Mouse4", expected: "sto_kb_space_mouse4" },
        {
          env: "ground",
          key: "Control+[",
          expected: "sto_kb_ground_control_leftbracket",
        }, // Special characters
        {
          env: "space",
          key: "Shift+]",
          expected: "sto_kb_space_shift_rightbracket",
        },
    ...
    it("should handle invalid key names gracefully", async () => {
      ...
      ); // Special characters converted to names
    });
  });
  ...
  describe("Export Generation", () => {
    it("should generate normal keybind export when bind-to-alias mode is disabled", async () => {
      ...
      const result = await exportService.generateKeybindSection(
      ...
      expect(result).toContain(
        'F1 "+STOTrayExecByTray 0 0 $$ +power_exec Distribute_Shields"',
      );
    ...
    it("should generate bind-to-alias export when mode is enabled", async () => {
      ...
      const keybindResult = await exportService.generateKeybindSection(
        profile.builds.space.keys,
        {
          environment: "space",
          profile,
      ...
      expect(keybindResult).toContain(
        "Keybind lines that call generated aliases",
      );
      expect(keybindResult).toContain(
        "(Alias definitions are in the alias file)",
      ...
      expect(aliasResult).toContain(
        "alias sto_kb_space_f1 <& +STOTrayExecByTray 0 0 $$ +power_exec Distribute_Shields &>",
      );
    });
    ...
    it("should handle different environments correctly in bind-to-alias mode", async () => {
      ...
      const aliasResult = await exportService.generateAliasFile(profile);
      expect(aliasResult).toContain(
        "alias sto_kb_ground_q <& target_enemy_near &>",
      );
    ...
    it("should handle stabilization in bind-to-alias mode", async () => {
      ...
      // Alias file should mirror only the TrayExec pre-pivot commands. Generic
      // commands remain before the palindrome and are not repeated.
      const aliasResult = await exportService.generateAliasFile(profile);
      expect(aliasResult).toContain(
        "alias sto_kb_space_q <& FireAll $$ +TrayExecByTray 0 0 $$ +TrayExecByTray 0 1 $$ +TrayExecByTray 0 0 &>",
      );
      expect(aliasResult).not.toContain("$$ FireAll &>");
    });
          ...
          if (action === "data:update-profile" && payload.modify) {
            ...
            if (payload.modify.bindsetMetadata) {
              ...
              for (const [bindsetName, bindsetData] of Object.entries(
                payload.modify.bindsetMetadata,
              )) {
  ...
  describe("Mode Switching", () => {
    it("should handle switching bind-to-alias mode on and off", async () => {
      ...
      expect(aliasKeybindResult).not.toContain(
        'alias sto_kb_space_q "FireAll"',
      ); // Aliases go to alias file
