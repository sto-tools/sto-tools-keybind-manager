...
describe("KeyBrowserUI Bindset Selection Tests", () => {
  ...
  beforeEach(() => {
    ...
    keyBrowserUI = new KeyBrowserUI(
      mockEventBus,
      dom.window.document.getElementById("key-browser"),
    );
  ...
  describe("createKeyElement with bindset context", () => {
    ...
    it("should NOT show key as selected when key name differs from selected key", () => {
      ...
      const keyElement1 = keyBrowserUI.createKeyElement(
        "F2",
        "Primary Bindset",
      );
  ...
  describe("bindset selection isolation regression tests", () => {
    it("should prevent selection bleeding between bindsets (regression: js-bindset-selection-state)", () => {
      ...
      const primaryBindsetElement = keyBrowserUI.createKeyElement(
        "F1",
        "Primary Bindset",
      );
      const customBindsetElement = keyBrowserUI.createKeyElement(
        "F1",
        "Custom Bindset",
      );
      ...
      keyBrowserUI.cache.activeBindset = "Custom Bindset";
      const primaryBindsetElement2 = keyBrowserUI.createKeyElement(
        "F1",
        "Primary Bindset",
      );
      const customBindsetElement2 = keyBrowserUI.createKeyElement(
        "F1",
        "Custom Bindset",
      );
    ...
    it("should maintain independent selection state across multiple bindsets", () => {
      ...
      const primaryF1_2 = keyBrowserUI.createKeyElement(
        "F1",
        "Primary Bindset",
      );
    ...
    it("should handle bindset context changes gracefully", () => {
      ...
      const scenarios = [
        {
          activeBindset: "Primary Bindset",
          context: "Primary Bindset",
          expected: true,
        },
        {
          activeBindset: "Primary Bindset",
          context: "Custom Bindset",
          expected: false,
        },
        {
          activeBindset: "Custom Bindset",
          context: "Primary Bindset",
          expected: false,
        },
        {
  ...
  describe("context-aware selection helper method", () => {
    it("isKeySelectedInContext should return correct boolean values", () => {
      ...
      expect(keyBrowserUI.isKeySelectedInContext("F1", "Primary Bindset")).toBe(
        true,
      );
      expect(keyBrowserUI.isKeySelectedInContext("F1", "Custom Bindset")).toBe(
        false,
      );

      // Test with non-matching key
      expect(keyBrowserUI.isKeySelectedInContext("F2", "Primary Bindset")).toBe(
        false,
      );
      expect(keyBrowserUI.isKeySelectedInContext("F2", "Custom Bindset")).toBe(
        false,
      );
