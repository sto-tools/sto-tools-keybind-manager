...
describe("ActivityTranslator", () => {
  describe("Constructor", () => {
    ...
    it("should merge custom options with defaults", () => {
      ...
      expect(customTranslator.options.validateUtf8).toBe(true); // Default preserved
    });
    ...
    it("should provide the same safe bindset sanitizer when no pipeline sanitizer is supplied", () => {
      expect(translator.sanitizeBindsetName(null)).toBe("unknown_bindset");
      expect(translator.sanitizeBindsetName("")).toBe("unnamed_bindset");
      expect(translator.sanitizeBindsetName("  12 My Build!  ")).toBe(
        "bs_12_my_build",
      );
      expect(translator.sanitizeBindsetName("***")).toBe("unnamed_bindset");
    });
  ...
  describe("generateActivityCommand - General Handling", () => {
    describe("Activity Handling - Fixed String Command Parameter Handling", () => {
      ...
      it("should generate fixed string command", () => {
        ...
        const result = translator.generateActivityCommand(
          FixedStringCommand,
          activityData,
          context,
        );
    ...
    describe("Activity Handling - Scale Type Command Parameter Handling", () => {
      ...
      it("should generate scale commands with valid n1 percentage", () => {
        ...
        expect(result.commands[0]).toBe(
          "GenSetEarlyOverrideFloat Hud_Statustargetfocused_Space_Bufflist scale 0.75$$GenSetEarlyOverrideFloat Hud_Statustargetfocused_Ground_Bufflist scale 0.75",
        );
      ...
      it("should use default scale (1.00) when n1 is missing or invalid", () => {
        ...
        testCases.forEach((activityData) => {
          ...
          expect(result.commands[0]).toBe(
            "GenSetEarlyOverrideFloat Hud_Statustargetfocused_Space_Bufflist scale 1.00$$GenSetEarlyOverrideFloat Hud_Statustargetfocused_Ground_Bufflist scale 1.00",
          );
          expect(result.type).toBe("parameterized_command");
        });
      });
    ...
    describe("General Handling - Unknown Activities", () => {
      ...
      it("should handle high activity IDs with special warnings", () => {
        ...
        const hasExpectedWarning = result.warnings.some((warning) =>
          warning.includes(
            "High activity ID 150 may indicate custom or future KBF format extension",
          ),
        );
        expect(hasExpectedWarning).toBe(true);
      });
    ...
    describe("General Handling - Error Categories and Suggestions", () => {
      ...
      it("should categorize out of range activities correctly", () => {
        expect(translator.determineActivityErrorCategory(-1)).toBe(
          "out_of_range",
        );
        expect(translator.determineActivityErrorCategory(124)).toBe(
          "out_of_range",
        );
      });
      ...
      it("should categorize future extension activities correctly", () => {
        ...
        expect(translator.determineActivityErrorCategory(150)).toBe(
          "out_of_range",
        );
      });

      it("should categorize unimplemented standard activities correctly", () => {
        expect(translator.determineActivityErrorCategory(30)).toBe(
          "unimplemented_standard",
        );
        expect(translator.determineActivityErrorCategory(123)).toBe(
          "unimplemented_standard",
        );
      });

      it("should categorize unknown basic activities correctly", () => {
        expect(translator.determineActivityErrorCategory(5)).toBe(
          "unknown_basic",
        );
      });

      it("should provide appropriate suggestions for different activity types", () => {
        expect(translator.getActivitySuggestion("string")).toBe(
          "Activity IDs should be numeric values",
        );
        expect(translator.getActivitySuggestion(-1)).toBe(
          "Valid activity IDs range from 0 to 123",
        );
        expect(translator.getActivitySuggestion(150)).toBe(
          "Valid activity IDs range from 0 to 123",
        );
        expect(translator.getActivitySuggestion(50)).toBe(
          "This activity may not be implemented yet in the current parser version",
        );
      });
    });
...
describe("generateActivityCommand - Bindset Loader", () => {
  describe("Activity 104 - Load Bind Set", () => {
    it("should generate bindset loader alias with valid text", () => {
      ...
      expect(result.commands).toEqual([
        "sto_kb_bindset_enable_space_My_Bindset",
      ]);
      expect(result.aliases).toEqual({});
    });
    ...
    it("should sanitize special characters in bindset name", () => {
      ...
      expect(result.commands).toEqual([
        "sto_kb_bindset_enable_ground_Combat_Binds_2_0",
      ]);
    });
    ...
    it("should support legacy context fields provided with activity data", () => {
      const activityData = { text: "Legacy Set", environment: "ground" };
      const result = translator.generateActivityCommand(104, activityData);

      expect(result.commands).toEqual([
        "sto_kb_bindset_enable_ground_Legacy_Set",
      ]);
    });

    it("should prefer explicit translation context over legacy context fields", () => {
      const activityData = { text: "Priority Set", environment: "space" };
      const context = { environment: "ground" };
      const result = translator.generateActivityCommand(
        104,
        activityData,
      ...
      expect(result.commands).toEqual([
        "sto_kb_bindset_enable_ground_Priority_Set",
      ]);
    });
    ...
    describe("Master to Primary Bindset mapping", () => {
      ...
      it('should map "Master" to "Primary Bindset" in ground environment', () => {
        ...
        expect(result.type).toBe("bindset_loader");
        expect(result.commands).toEqual([
          "sto_kb_bindset_enable_ground_Primary_Bindset",
        ]);
        expect(result.aliases).toEqual({});
      });
      ...
      it('should handle case-insensitive "master" mapping', () => {
        ...
        testCases.forEach(({ text, expected }) => {
          ...
          expect(result.commands).toEqual([
            `sto_kb_bindset_enable_space_${expected}`,
          ]);
        });
      });
      ...
      it("should not affect other bindset names", () => {
        ...
        testCases.forEach((bindsetName) => {
          ...
          expect(result.commands).toEqual([
            `sto_kb_bindset_enable_ground_${expectedSanitized}`,
          ]);
        });
      });
      ...
      it("should preserve existing behavior for non-Master bindsets", () => {
        ...
        expect(result.type).toBe("bindset_loader");
        expect(result.commands).toEqual([
          "sto_kb_bindset_enable_space_Custom_Bindset",
        ]);
        expect(result.aliases).toEqual({});
      });
    });
  ...
  describe("generateActivityCommand - Text Commands", () => {
    ...
    describe("Activity 97 - EmoteCycle", () => {
      ...
      it("should handle empty emote cycle text", () => {
        ...
        expect(result.aliases["sto_kb_emotecycle_Q_2_step0"].commands).toEqual([
          "emote_notext ",
        ]);
      });
      ...
      it("should use context defaults for alias naming", () => {
        ...
        expect(
          result.aliases["sto_kb_emotecycle_key_0_step0"].commands,
        ).toEqual(["emote_notext cheer"]);
      });
    });
    ...
    describe("Activity 99 - bind_load_file", () => {
      ...
      it("should handle filename with spaces", () => {
        ...
        expect(result.commands).toEqual([
          "bind_load_file pvp healing setup.txt",
        ]);
      });
    ...
    describe("Activity 101 - cycle_emote", () => {
      ...
      it("should handle empty text gracefully", () => {
        ...
        expect(result.type).toBe("cycle_command");
        expect(
          result.aliases["sto_kb_emotecyclevisible_F_1_step0"].commands,
        ).toEqual(["emote "]);
      });
      ...
      it("should handle missing context gracefully", () => {
        ...
        expect(
          result.aliases["sto_kb_emotecyclevisible_key_0_step0"].commands,
        ).toEqual(["emote dance"]);
      });
  ...
  describe("generateActivityCommand - Parameterized Commands", () => {
    ...
    describe("Activity 26 - TrayExecByTray - Full Tray", () => {
      const buildTraySequence = (tray) =>
        Array.from(
          { length: 10 },
          (_, slot) => `+TrayExecByTray ${tray} ${slot}`,
        );
    ...
    describe("Activity 67 - rdrMaxFramesAhead", () => {
      ...
      it("should process numeric values", () => {
        ...
        // Test integer
        const intResult = translator.generateActivityCommand(
          67,
          { n1: 4 },
          context,
        );
        ...
        // Test decimal
        const decimalResult = translator.generateActivityCommand(
          67,
          { n1: 3.7 },
          context,
        );
        ...
        // Test zero
        const zeroResult = translator.generateActivityCommand(
          67,
          { n1: 0 },
          context,
        );
    ...
    describe("Activity 86 - GenSetEarlyOverrideFloat Hud_Statustargetoftarget_Space_Bufflist scale", () => {
      it("should generate scale commands with valid n1 percentage", () => {
        ...
        // Should generate two commands joined by $$ for space and ground
        expect(result.commands[0]).toBe(
          "GenSetEarlyOverrideFloat Hud_Statustargetoftarget_Space_Bufflist scale 1.50$$GenSetEarlyOverrideFloat Hud_Statustargetoftarget_Ground_Bufflist scale 1.50",
        );
        expect(result.type).toBe("parameterized_command");
        expect(result.aliases).toEqual({});
      });
    ...
    describe("Activity 87 - GenSetEarlyOverrideFloat Hud_Statustarget_Space_Bufflist scale", () => {
      it("should generate scale commands with valid n1 percentage", () => {
        ...
        // Should generate two commands joined by $$ for space and ground
        expect(result.commands[0]).toBe(
          "GenSetEarlyOverrideFloat Hud_Statustarget_Space_Bufflist scale 1.20$$GenSetEarlyOverrideFloat Hud_Statustarget_Ground_Bufflist scale 1.20",
        );
        expect(result.type).toBe("parameterized_command");
        expect(result.aliases).toEqual({});
      });
    ...
    describe("Activity 89 - GenSetEarlyOverrideFloat Hud_Statusself_Bufflist scale", () => {
      it("should generate PersonalBuffScale command with default scale", () => {
        ...
        expect(result.commands).toHaveLength(1);
        expect(result.commands[0]).toBe(
          "GenSetEarlyOverrideFloat Hud_Statusself_Bufflist scale 1.00",
        );
        expect(result.aliases).toEqual({});
      });
    });
    ...
    describe("Activity 91 - GenSetEarlyOverrideFloat Hud_Statustargetoftarget_Space_Bufflist scale", () => {
      it("should generate AllBuffScale commands with default scale", () => {
        ...
        expect(command).toContain(
          "GenSetEarlyOverrideFloat Hud_Statustargetoftarget_Space_Bufflist scale 1.00",
        );
        expect(command).toContain(
          "GenSetEarlyOverrideFloat Hud_Statustargetoftarget_Ground_Bufflist scale 1.00",
        );
        expect(command).toContain(
          "GenSetEarlyOverrideFloat Hud_Statustarget_Space_Bufflist scale 1.00",
        );
        expect(command).toContain(
          "GenSetEarlyOverrideFloat Hud_Statustarget_Ground_Bufflist scale 1.00",
        );
        expect(command).toContain(
          "GenSetEarlyOverrideFloat Hud_Statustargetfocused_Space_Bufflist scale 1.00",
        );
        expect(command).toContain(
          "GenSetEarlyOverrideFloat Hud_Statustargetfocused_Ground_Bufflist scale 1.00",
        );
        expect(command).toContain(
          "GenSetEarlyOverrideFloat Hud_Statusself_Bufflist scale 1.00",
        );
    ...
    describe("Activity 92 - GenSetEarlyOverrideFloat Hud_Spacetraywindow_Large scale", () => {
      it("should generate ScaleSpaceTray command with default scale 100%", () => {
        ...
        expect(result.type).toBe("parameterized_command");
        expect(result.commands).toEqual([
          "GenSetEarlyOverrideFloat Hud_Spacetraywindow_Large scale 1.00",
        ]);
        expect(result.aliases).toEqual({});
      });
    });
    ...
    describe("Activity 93 - GenSetEarlyOverrideFloat Hud_Buffs scale", () => {
      it("should generate SpacePersonalBuffScale command with default scale 100%", () => {
        ...
        expect(result.type).toBe("parameterized_command");
        expect(result.commands).toEqual([
          "GenSetEarlyOverrideFloat Hud_Buffs scale 1.00",
        ]);
        expect(result.aliases).toEqual({});
      });
    });
    ...
    describe("Activity 94 - GenSetEarlyOverrideFloat Hud_Spacetraywindow_Secondary 1.00", () => {
      it("should generate SpaceVerticleTrayScale command with default scale 100%", () => {
        ...
        expect(result.type).toBe("parameterized_command");
        expect(result.commands).toEqual([
          "GenSetEarlyOverrideFloat Hud_Spacetraywindow_Secondary 1.00",
        ]);
        expect(result.aliases).toEqual({});
      });
    });
    ...
    describe("Activity 116 - GenSliderSetNotch Doff_Recipe_Quantity_Slider", () => {
      ...
      it("should generate correct command with custom n1 value", () => {
        ...
        expect(result.commands).toEqual([
          "GenSliderSetNotch Doff_Recipe_Quantity_Slider 50",
        ]);
        expect(result.type).toBe("parameterized_command");
      });
    });
    ...
    describe("Activity 121 - trayelemdestroy", () => {
      ...
      it("should handle edge cases and default values", () => {
        const testCases = [
          ...
          {
            activityData: { text: "ignored" },
            expected: "trayelemdestroy 0 0",
          },
          {
            activityData: { n1: null, n2: null },
            expected: "trayelemdestroy 0 0",
          },
          {
            activityData: { n1: undefined, n2: undefined },
            expected: "trayelemdestroy 0 0",
          },
      ...
      it("should generate loop of trayelemdestroy commands for all slots 0-9", () => {
        ...
        testCases.forEach((testCase) => {
          ...
          for (let slotNum = 0; slotNum <= 9; slotNum++) {
            expect(result.commands[slotNum]).toBe(
              `trayelemdestroy ${testCase.expectedTray} ${slotNum}`,
            );
          }
        });
      });
