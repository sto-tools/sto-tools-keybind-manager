...
describe("FieldParser - Key Token Normalization", () => {
  ...
  describe("normalizeKeyToken method", () => {
    ...
    it("should handle special case variations", () => {
      ...
      expect(fieldParser.normalizeKeyToken("backspace")).toBe("Backspace");
    ...
    it("should return original token for unknown keys", () => {
      ...
      expect(fieldParser.normalizeKeyToken("SomeSpecialKey")).toBe(
        "SomeSpecialKey",
      );
    });
  ...
  describe("parseKeyTokenField method with normalization", () => {
    ...
    it("should handle special tokens correctly", () => {
      ...
      // Test Space special case
      const spaceRecord = {
        hasColon: true,
        value: "Space",
      };
      expect(fieldParser.parseKeyTokenField(spaceRecord, 0)).toBe("Space");
    });

    it("should preserve the canonical Space key name", () => {
      const spaceRecord = {
        hasColon: true,
        value: "Space",
      };
  ...
  describe("Regression tests", () => {
    it("should fix the original KBF uppercase key token bug (regression: js-kbf-key-token-normalization)", () => {
      ...
      expect(
        fieldParser.parseKeyTokenField({ hasColon: true, value: "SPACE" }, 0),
      ).toBe("Space");
      expect(
        fieldParser.parseKeyTokenField({ hasColon: true, value: "CTRL" }, 0),
      ).toBe("Control");
      expect(
        fieldParser.parseKeyTokenField({ hasColon: true, value: "CONTROL" }, 0),
      ).toBe("Control");
      expect(
        fieldParser.parseKeyTokenField({ hasColon: true, value: "ALT" }, 0),
      ).toBe("ALT"); // ALT is already correct
      expect(
        fieldParser.parseKeyTokenField({ hasColon: true, value: "SHIFT" }, 0),
      ).toBe("Shift");
      expect(
        fieldParser.parseKeyTokenField({ hasColon: true, value: "TAB" }, 0),
      ).toBe("Tab");
    });
  });
});
