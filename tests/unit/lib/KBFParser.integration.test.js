...
describe("KBFParser - Real File Integration Tests", () => {
  ...
  describe("Real KBF File Parsing", () => {
    ...
    it("should preserve generated cycle aliases from a real KBF file", () => {
      const keysetFile = readFileSync(join(fixturesPath, "keyset.KBF"), "utf8");
      ...
      const spaceCommands = result.bindsets.master.keys.Space.commands;
      const controllerName = spaceCommands.find((command) =>
        command.startsWith("sto_kb_emotecycle_"),
      );

      expect(controllerName).toBeDefined();
      expect(result.aliases).toHaveProperty(controllerName);
      expect(result.aliases).toHaveProperty(`${controllerName}_step0`);
      expect(result.aliases[`${controllerName}_step0`].commands).toEqual([
        "emote_notext ",
      ]);
      expect(result.stats.totalAliases).toBe(2);
    });

    it("should parse no-keyset.KBF without errors", () => {
      const noKeysetFile = readFileSync(
        join(fixturesPath, "no-keyset.KBF"),
        "utf8",
      );
