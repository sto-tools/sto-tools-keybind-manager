...
describe("aliasNameValidator", () => {
  ...
  describe("generateBindToAliasName", () => {
    ...
    it("converts special characters to meaningful names", () => {
      expect(generateBindToAliasName("ground", "Control+[")).toBe(
        "sto_kb_ground_control_leftbracket",
      );
      expect(generateBindToAliasName("space", "Shift+]")).toBe(
        "sto_kb_space_shift_rightbracket",
      );
      expect(generateBindToAliasName("space", "Alt+=")).toBe(
        "sto_kb_space_alt_equals",
      );
      expect(generateBindToAliasName("ground", "Ctrl+-")).toBe(
        "sto_kb_ground_ctrl_minus",
      );
      expect(generateBindToAliasName("space", "Shift+(")).toBe(
        "sto_kb_space_shift_leftparen",
      );
      expect(generateBindToAliasName("space", "Alt+)")).toBe(
        "sto_kb_space_alt_rightparen",
      );
    });

    it("handles complex key combinations", () => {
      expect(generateBindToAliasName("space", "Ctrl+Shift+A")).toBe(
        "sto_kb_space_ctrl_shift_a",
      );
    ...
    it("sanitizes environment names", () => {
      expect(generateBindToAliasName("Ground-Control", "A")).toBe(
        "sto_kb_groundcontrol_a",
      );
    ...
    it("collapses multiple underscores and removes leading/trailing ones", () => {
      expect(generateBindToAliasName("space", "Ctrl+_+A")).toBe(
        "sto_kb_space_ctrl_underscore_a",
      );
      expect(generateBindToAliasName("space", "___test___")).toBe(
        "sto_kb_space_underscoreunderscoreunderscoretestunderscoreunderscoreunderscore",
      );
    });
  });
});
