...
describe("Command Display Adapter", () => {
  ...
  beforeEach(() => {
    ...
    detachParserHandler = respond(
      eventBus,
      "parser:parse-command-string",
      ({ commandString }) => {
        // Simple mock that creates a command object from the string
        if (
          commandString.includes("TrayExecByTray") &&
          !commandString.includes("WithBackup")
        ) {
          ...
          return {
            commands: [
              {
                command: commandString,
                signature: commandString.includes("TrayExecByTray")
                  ? "TrayExecByTray"
                  : "BasicCommand",
                parameters: commandString.includes("TrayExecByTray")
                  ? { tray: 0, slot: 0 }
                  : {},
                displayText: commandString,
                // Return 'custom' for unknown/empty commands, 'combat' for known ones
                category:
                  commandString === "FireAll" ||
                  commandString.includes("TrayExecByTray")
                    ? "combat"
                    : "custom",
                id: `mock_${Date.now()}`,
  ...
  describe("enrichForDisplay", () => {
    ...
    it("should handle commands without definitions", async () => {
      const result = await enrichForDisplay("UnknownCommand", mockI18n, {
        eventBus,
      });
  ...
  describe("normalizeToStringArray", () => {
    ...
    it("should handle single item (not array)", () => {
      ...
      const result2 = normalizeToStringArray({
        command: "FireAll",
        name: "Fire All",
      });
  ...
  describe("integration scenarios", () => {
    it("should handle complete workflow: rich objects to strings and back", async () => {
      ...
      const enrichedObjects = await Promise.all(
        canonicalStrings.map((cmd) =>
          enrichForDisplay(cmd, mockI18n, { eventBus }),
        ),
      );
    ...
    it("should preserve data integrity through multiple conversions", () => {
      ...
      const asRichObjects = originalCommands.map((cmd) => ({
        command: cmd,
        name: `Name for ${cmd}`,
      }));
  ...
  describe("normalizeToOptimizedString", () => {
    ...
    it("should keep TrayExecByTray commands with active=0 in explicit form", async () => {
      const result = await normalizeToOptimizedString("TrayExecByTray 0 3 0", {
        eventBus,
      });
    ...
    it("should optimize TrayExecByTrayWithBackup commands with active=1 to + form", async () => {
      const result = await normalizeToOptimizedString(
        "TrayExecByTrayWithBackup 1 3 0 4 1",
        { eventBus },
      );
    ...
    it("should keep TrayExecByTrayWithBackup commands with active=0 in explicit form", async () => {
      const result = await normalizeToOptimizedString(
        "TrayExecByTrayWithBackup 0 3 0 4 1",
        { eventBus },
      );
