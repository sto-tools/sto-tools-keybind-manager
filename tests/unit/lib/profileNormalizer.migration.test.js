...
describe("Profile Migration System", () => {
  ...
  describe("VFX Alias Migration (2.0.0 -> 2.1.0)", () => {
    ...
    it("should remove VFX aliases with dynFxSetFXExclusionList names", () => {
      ...
      expect(result.aliases).toHaveProperty("normalAlias");
      expect(result.aliases).not.toHaveProperty(
        "dynFxSetFXExclusionList_Space",
      );
      expect(result.aliases).not.toHaveProperty(
        "dynFxSetFXExclusionList_Ground",
    ...
    it("should preserve other aliases during VFX migration", () => {
      ...
      expect(result.aliases.userAlias1.commands).toEqual([
        "Command1",
        "Command2",
      ]);
  ...
  describe("Legacy Command Normalization", () => {
    it("should still normalize legacy commands alongside migration", () => {
      ...
      expect(result.aliases.legacyAlias.commands).toEqual([
        "Command1",
        "Command2",
      ]);
  ...
  describe("Real-world Migration Scenarios", () => {
    it("should handle a typical user profile with VFX aliases", () => {
      ...
      expect(result.aliases.AttackSequence.commands).toEqual([
        "TargetEnemyNear",
        "FireAll",
      ]);
  ...
  describe("VFX Command Spelling Migration (2.1.0 -> 2.1.1)", () => {
    it("should fix VFX command spelling in VFX Manager aliases", () => {
      const profile = {
        ...
        aliases: {
          dynFxSetFXExclusionList_Space: {
            commands: [
              "dynFxSetFXExclusionList FX_SpaceEffect1,FX_SpaceEffect2",
            ],
          },
          dynFxSetFXExclusionList_Ground: {
            commands: [
              "dynFxSetFXExclusionList FX_GroundEffect1 $$ PlayerSay VFX Loaded",
            ],
          },
          dynFxSetFXExclusionList_Combined: {
      ...
      expect(result.aliases.dynFxSetFXExclusionList_Space.commands).toEqual([
        "dynFxSetFXExlusionList FX_SpaceEffect1,FX_SpaceEffect2",
      ]);
      expect(result.aliases.dynFxSetFXExclusionList_Ground.commands).toEqual([
        "dynFxSetFXExlusionList FX_GroundEffect1 $$ PlayerSay VFX Loaded",
      ]);
      expect(result.aliases.dynFxSetFXExclusionList_Combined.commands).toEqual([
        "dynFxSetFXExlusionList FX_SpaceEffect,FX_GroundEffect",
      ]);

      // User aliases should NOT be affected
      expect(result.aliases.UserAlias.commands).toEqual([
        "dynFxSetFXExclusionList SomeOtherEffect",
      ]);
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith(
        expect.stringContaining(
          "Fixing VFX command spelling in alias: dynFxSetFXExclusionList_Space",
        ),
      );
      expect(consoleLogSpy).toHaveBeenCalledWith(
        expect.stringContaining(
          "Fixing VFX command spelling in alias: dynFxSetFXExclusionList_Ground",
        ),
      );
      expect(consoleLogSpy).toHaveBeenCalledWith(
        expect.stringContaining(
          "Fixing VFX command spelling in alias: dynFxSetFXExclusionList_Combined",
        ),
      );
    });
    ...
    it("should not affect profiles without VFX aliases", () => {
      ...
      expect(result.aliases.UserVFXAlias.commands).toEqual([
        "dynFxSetFXExclusionList SomeEffect",
      ]);
    ...
    it("should not affect profiles already on version 2.1.1", () => {
      ...
      expect(result.aliases.dynFxSetFXExclusionList_Space.commands).toEqual([
        "dynFxSetFXExclusionList WrongSpelling",
      ]);
      expect(result.migrationVersion).toBe("2.1.1");
