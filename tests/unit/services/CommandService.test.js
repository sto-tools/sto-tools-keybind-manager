...
describe("CommandService mutations", () => {
  let fixture, eventBus, service, profile, detachUpdate;
  ...
  beforeEach(() => {
    ...
    detachUpdate = respond(
      eventBus,
      "data:update-profile",
      ({ add, modify, delete: del }) => {
  ...
  it("deleteCommand should remove entry and emit command-deleted", async () => {
    ...
    expect(spy).toHaveBeenCalledWith(
      expect.objectContaining({ key: "F1", index: 0 }),
    );
    eventBus.emit("profile:switched", {
      profileId: profile.id,
      profile,
  ...
  it("moveCommand should reorder and emit command-moved", async () => {
    ...
    await new Promise((r) => setTimeout(r, 0));
    expect(spy).toHaveBeenCalledWith(
      expect.objectContaining({ key: "F1", fromIndex: 0, toIndex: 2 }),
    );
    eventBus.emit("profile:switched", {
      profileId: profile.id,
      profile,
