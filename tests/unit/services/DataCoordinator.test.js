...
describe("DataCoordinator Service", () => {
  ...
  describe("Profile Operations", () => {
    it("should create new profile", async () => {
      const result = await dataCoordinator.createProfile(
        "New Profile",
        "Test Description",
        "space",
      );
      ...
      expect(dataCoordinator.state.profiles[result.profileId].description).toBe(
        "Test Description",
      );
      expect(
        dataCoordinator.state.profiles[result.profileId].keybindMetadata,
      ).toEqual({ space: {}, ground: {} });
      expect(
        dataCoordinator.state.profiles[result.profileId].aliasMetadata,
      ).toEqual({});
      expect(
        dataCoordinator.state.profiles[result.profileId].bindsetMetadata,
      ).toEqual({});
    });
    ...
    it("should fail to switch to non-existent profile", async () => {
      await expect(
        dataCoordinator.switchProfile("non-existent"),
    ...
    it("should clone existing profile", async () => {
      ...
      expect(clonedProfile.aliases).toEqual({
        test: { commands: ["command1"], description: "Test alias" },
      });
    });
    ...
    it("should rename existing profile", async () => {
      ...
      expect(result.success).toBe(true);
      expect(dataCoordinator.state.profiles["test-profile"].name).toBe(
        "New Name",
      );
      expect(dataCoordinator.state.profiles["test-profile"].description).toBe(
        "New Description",
      );
    });
    ...
    it("should not delete last remaining profile", async () => {
      ...
      await expect(dataCoordinator.deleteProfile("profile1")).rejects.toThrow(
  ...
  describe("Error Handling", () => {
    ...
    it("should handle invalid profile operations", async () => {
      await expect(
        dataCoordinator.updateProfile("non-existent", {
          properties: { description: "Updated" },
        }),
      ).rejects.toThrow("Profile non-existent not found");
    });
    ...
    it("should handle null updates parameter correctly", async () => {
      ...
      await expect(
        dataCoordinator.updateProfile("test-profile", null),
    ...
    it("should handle undefined updates parameter correctly", async () => {
      ...
      await expect(
        dataCoordinator.updateProfile("test-profile", undefined),
      ).rejects.toThrow("Updates are required");
    });
    ...
    it("should handle null profile ID correctly", async () => {
      ...
      await expect(
        dataCoordinator.updateProfile(null, {
          properties: { description: "Updated" },
        }),
    ...
    it("should handle undefined profile ID correctly", async () => {
      ...
      await expect(
        dataCoordinator.updateProfile(undefined, {
          properties: { description: "Updated" },
        }),
      ).rejects.toThrow("Profile ID is required");
    });
