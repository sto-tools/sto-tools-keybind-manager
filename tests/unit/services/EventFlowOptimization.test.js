...
describe("Phase 2.1: Event Flow Optimization - keys:changed Elimination", () => {
  ...
  describe("KeyBrowserService still receives key updates", () => {
    it("should emit key:list-changed in response to profile:updated (direct flow)", async () => {
      ...
      console.log(
        "KeyBrowserService cache.currentProfile before:",
        keyBrowserService.cache.currentProfile,
      );

      // Check if KeyBrowserService has the event listener registered
      console.log(
        "EventBus listener count for profile:updated:",
        harness.eventBus.getListenerCount("profile:updated"),
      );
      ...
      console.log(
        "Captured events after emission:",
        capturedEvents.map((e) => e.event),
      );
      console.log(
        "KeyBrowserService current profile after:",
        keyBrowserService.cache.currentProfile,
      );
      ...
      const keyListChangedEvents = capturedEvents.filter(
        (e) => e.event === "key:list-changed",
      );
      const keysChangedEvents = capturedEvents.filter(
        (e) => e.event === "keys:changed",
      );
  ...
  describe("Event flow optimization working correctly", () => {
    it("should have direct DataCoordinator → KeyBrowserService flow", async () => {
      ...
      const serviceResponseEvents = capturedEvents.filter(
        (e) =>
  ...
  describe("Performance improvement verification", () => {
    it("should emit fewer events per profile operation", async () => {
      ...
      const relevantEvents = capturedEvents.filter(
        (e) => e.event === "key:list-changed" || e.event === "keys:changed",
      );
      ...
      expect(relevantEvents.every((e) => e.event === "key:list-changed")).toBe(
        true,
      );
      expect(
        relevantEvents.filter((e) => e.event === "keys:changed"),
      ).toHaveLength(0);
    });
  });
});
