...
describe("ImportService", () => {
  ...
  describe("importProjectFile", () => {
    ...
    it("should fail when storage rejects a profile write", async () => {
      ...
      expect(result).toEqual({
        ...
        params: { reason: 'Storage rejected profile "rejected"' },
      });
      expect(fixture.storage.saveAllData).not.toHaveBeenCalled();
    });

    it("should fail when storage rejects the settings write", async () => {
      ...
      expect(result).toEqual({
        success: false,
        error: "import_failed_invalid_json",
        params: { reason: "Storage rejected project settings" },
      });
    });

    it("should fail when storage rejects restored project state", async () => {
      ...
      expect(result).toEqual({
        success: false,
        error: "import_failed_invalid_json",
        params: { reason: "Storage rejected restored project state" },
      });
    });
    ...
    it("should handle missing sanitizeProfileData method gracefully", async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.error).toBe("import_failed_invalid_json");
      expect(result.params.reason).toContain(
        "sanitizeProfileData is not a function",
      );
  ...
  describe("importKBFFile", () => {
    ...
    it("should return no valid bindsets found when parser returns empty bindsets", async () => {
      ...
      expect(result.message).toContain(
        "KBF file contains no valid bindsets that could be imported",
      );
      expect(result.errors).toBeDefined();
      expect(result.warnings).toBeDefined();
  ...
  describe("import:kbf-file endpoint", () => {
    ...
    it("should handle parsing errors via request endpoint", async () => {
      ...
      vi.spyOn(service.kbfParser, "parseFile").mockRejectedValue(
        new Error("Layer 3 decoding failed"),
      );
    ...
    it("should handle partial import with warnings via request endpoint", async () => {
      ...
      expect(
        result.warnings.some((w) =>
          typeof w === "string"
            ? w.includes("Unknown activity type 150")
            : w.message.includes("Unknown activity type 150"),
        ),
      ).toBe(true);
      expect(
        result.warnings.some((w) =>
          typeof w === "string"
            ? w.includes("UnknownKey")
            : w.message.includes("UnknownKey"),
        ),
      ).toBe(true);
...
alias existing_alias1 "replace_existing"
      ...
      it("should implement merge_keep strategy for aliases", async () => {
        ...
        const profile = fixture.storage.getProfile("test-profile");
        expect(profile.aliases["existing_alias1"]).toEqual({
          commands: "existing_alias_cmd1",
        }); // unchanged
      });
      ...
      it("should implement merge_overwrite strategy for aliases", async () => {
        ...
        const profile = fixture.storage.getProfile("test-profile");
        expect(profile.aliases["existing_alias1"]).not.toEqual({
          commands: "existing_alias_cmd1",
        }); // replaced
      });
