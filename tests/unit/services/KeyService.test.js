...
describe("KeyService", () => {
  let fixture, eventBus, keyService, profileFixture;
  ...
  beforeEach(() => {
    ...
    keyService.updateCacheFromProfile = vi.fn((profile) => {
      if (!profile) return;
      keyService.cache.builds = profile.builds || {
        space: { keys: {} },
        ground: { keys: {} },
      };
      keyService.cache.keys =
        keyService.cache.builds[keyService.cache.currentEnvironment]?.keys ||
        {};
    ...
    keyService.isValidKeyName = vi.fn(async (keyName) => {
      return (
        keyService.validKeys.includes(keyName) && keyName.trim().length > 0
      );
    });
  ...
  describe("Event Integration", () => {
    ...
    it("should NOT emit keys:changed when deleting keys (Phase 2.2)", async () => {
      ...
      keyService.deleteKey = vi.fn(async (keyName) => {
        if (
          !keyService.cache.currentProfile ||
          !keyService.cache.keys[keyName]
        ) {
          return false;
        }
  ...
  describe("State Management", () => {
    it("should maintain cache consistency", () => {
      ...
      expect(keyService.cache.keys).toEqual(
        profileFixture.profile.builds.ground.keys,
      );
    });
