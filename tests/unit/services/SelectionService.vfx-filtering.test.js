...
describe("SelectionService VFX Alias Filtering", () => {
    ...
    selectionService.cache = {
      ...
      aliases: {
        ...
        UserAlias2: {
          commands: "Shield",
          type: "alias",
          description: "Another user alias",
        },
  ...
  describe("VFX Alias Filtering in Auto-Selection", () => {
    ...
    it("should validate only user-created aliases, not VFX system aliases", () => {
      ...
      expect(
        selectionService.validateAliasExists("dynFxSetFXExclusionList_Space"),
      ).toBe(false);
      expect(
        selectionService.validateAliasExists("dynFxSetFXExclusionList_Ground"),
      ).toBe(false);
    });
    ...
    it("should not restore cached VFX system alias selections", async () => {
      ...
      await selectionService.validateAndRestoreSelection(
        "alias",
        "dynFxSetFXExclusionList_Space",
      );
      ...
      expect(["UserAlias1", "UserAlias2", "UserAlias3"]).toContain(
        selectedAlias,
      );
    });
    ...
    it("should correctly filter aliases in the same way as AliasBrowserService", () => {
      ...
      expect(Object.keys(userAliases)).toEqual([
        "UserAlias1",
        "UserAlias2",
        "UserAlias3",
      ]);
