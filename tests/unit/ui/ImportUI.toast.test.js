...
describe("ImportUI Toast Tests", () => {
  ...
  beforeEach(() => {
    fixture = createUIComponentFixture(ImportUI, {
      i18n: {
        t: vi.fn((key, params) => {
          if (key === "import_success_keys")
            return `Successfully imported ${params.count} keys`;
          if (key === "import_success_aliases")
            return `Successfully imported ${params.count} aliases`;
  ...
  describe("keybind import success", () => {
    it("should show success toast when keybind import succeeds", async () => {
      ...
      if (result?.success) {
        const message = component.i18n?.t?.(result?.message, {
          count: result.imported?.keys || 0,
        });
  ...
  describe("alias import success", () => {
    it("should show success toast when alias import succeeds", async () => {
      ...
      if (result?.success) {
        const message = component.i18n?.t?.(result?.message, {
          count: result.imported?.aliases || 0,
        });
        component.showToast(message, "success");
      }
  ...
  describe("keybind import failure", () => {
    it("should show error toast when keybind import fails", async () => {
      ...
      expect(showToastSpy).toHaveBeenCalledWith("Invalid file format", "error");
    });
  });
  ...
  describe("bindset selection modal", () => {
    it("should create bindset selection modal with checkboxes", () => {
      ...
      const modal = component.createBindsetSelectionModal(
        bindsetNames,
        hasMasterBindset,
        masterDisplayName,
      );
  ...
  describe("KBF Import - Instant Toast Behavior (Post-Fix)", () => {
    ...
    it("should behave consistently with other import types", async () => {
      ...
      if (keybindResult?.success) {
        const message = component.i18n?.t?.(keybindResult?.message, {
          count: keybindResult.imported?.keys || 0,
        });
        component.showToast(message, "success");
      }
      ...
      if (aliasResult?.success) {
        const message = component.i18n?.t?.(aliasResult?.message, {
          count: aliasResult.imported?.aliases || 0,
        });
        component.showToast(message, "success");
      }
      ...
      if (kbfResult?.success) {
        const message =
          component.i18n?.t?.(kbfResult?.message) || kbfResult?.message;
        component.showToast(message, "success");
      }
  ...
  describe("ImportUI Strategy Selection and Confirmation Logic", () => {
      ...
      fixture = createUIComponentFixture(ImportUI, {
        i18n: {
          t: vi.fn((key, params) => {
            ...
            if (key === "merge_overwrite_existing")
              return "Merge (overwrite existing)";
            if (key === "overwrite_all") return "Overwrite all";
            if (key === "import") return "Import";
            if (key === "cancel") return "Cancel";
            if (key === "import_result_skipped")
              return `Imported ${params.imported}, skipped ${params.skipped} conflicts.`;
            if (key === "import_result_overwrote")
              return `Imported ${params.imported}, overwrote ${params.overwritten} items.`;
            if (key === "import_result_overwrite_all")
              return `Imported ${params.imported} after clearing ${params.cleared} existing items.`;
            if (key === "overwrite_confirm_title") return "Confirm overwrite";
            if (key === "overwrite_confirm_body_keys")
              return `This will remove all existing keybinds in ${params.environment} and replace them with the import file.`;
            if (key === "overwrite_confirm_body_aliases")
              return "This will remove all existing aliases and replace them with the import file.";
    ...
    describe("Strategy Radio Groups", () => {
      ...
      it("should extract strategy from environment modal selection", async () => {
        ...
        component.promptEnvironment = vi.fn(() =>
    ...
    describe("Overwrite Confirmation Dialog", () => {
      it("should create overwrite confirmation modal for keybinds", () => {
        const modal = component.createOverwriteConfirmationModal(
          "keys",
          5,
          3,
          "space",
        ...
        expect(modal.innerHTML).toContain(
          "This will remove all existing keybinds in space",
        );
      ...
      it("should create overwrite confirmation modal for aliases", () => {
        ...
        expect(modal.innerHTML).toContain("Confirm overwrite");
        expect(modal.innerHTML).toContain(
          "This will remove all existing aliases",
        );
      ...
      it("should return true when user confirms overwrite", async () => {
        ...
        component.showOverwriteConfirmation = vi.fn(() =>
          Promise.resolve(true),
        );

        const result = await component.showOverwriteConfirmation(
          "keys",
          5,
          3,
          "space",
    ...
    describe("Strategy Toast Messages", () => {
      it("should show correct toast message for merge_keep strategy with skipped items", () => {
        ...
        if (cleared > 0) {
          ...
          const message = component.i18n.t(messageKey, {
            imported,
            overwritten,
          });
          component.showToast(message, "success");
