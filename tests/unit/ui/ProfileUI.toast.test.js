...
describe("ProfileUI Toast Tests", () => {
  ...
  beforeEach(() => {
    fixture = createUIComponentFixture(ProfileUI, {
      ...
      i18n: {
        t: vi.fn((key) => {
          if (key === "no_profile_selected_to_clone")
            return "No profile selected to clone";
          if (key === "no_profile_selected_to_rename")
            return "No profile selected to rename";
          if (key === "profile_name_required") return "Profile name required";
          if (key === "profile_renamed") return "Profile renamed";
          if (key === "no_profile_selected_to_delete")
            return "No profile selected to delete";
          return key;
        }),
      },
      document: {
